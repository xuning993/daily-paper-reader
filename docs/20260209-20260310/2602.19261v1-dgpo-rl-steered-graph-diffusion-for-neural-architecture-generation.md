...
## 速览
**TLDR**：本研究针对神经网络架构搜索（NAS）中现有图扩散模型忽略有向无环图（DAG）方向性的问题，提出了DGPO框架。该方法通过拓扑排序和位置编码，将强化学习微调引入离散图扩散模型，使其能够捕捉数据流语义。实验证明，DGPO在NAS-Bench-201所有任务上均达到最优性能，且展现出极强的结构先验迁移能力，仅需少量数据预训练即可通过奖励引导生成高质量架构，为有向组合结构的受控生成提供了新方案。 \
**Motivation**：现有图扩散模型在生成神经网络架构时忽略了有向无环图（DAG）的边方向信息，导致无法有效建模数据流语义。 \
**Method**：提出DGPO框架，利用拓扑排序和位置编码将强化学习微调扩展至离散图扩散模型，以实现对DAG结构的精确建模。 \
**Result**：在NAS-Bench-201上达到基准最优性能，并证明模型在仅使用7%数据预训练的情况下仍能通过微调生成接近理想的架构。 \
**Conclusion**：强化学习引导的离散扩散模型在处理有向性后，能为有向组合结构提供高效且可控的生成能力。
