...
## 速览
**TLDR**：本研究针对统一设施选址问题（UniFL），提出了一种融合近似算法原理的全微分图神经网络（MPNN）模型。该方法克服了现有学习算法缺乏性能保证及训练不稳定的缺陷，无需求解器监督即可实现端到端训练。模型具备可证明的近似比和规模泛化能力，在实验中表现优于传统启发式算法，显著缩小了与整数线性规划的性能差距，为结合深度学习与离散优化提供了新思路。 \
**Motivation**：现有组合优化学习方法缺乏性能保证且训练开销大，而传统近似算法虽有保证但无法自适应数据分布。 \
**Method**：提出一种全微分的消息传递神经网络模型，将近似算法原理嵌入架构中，无需求解器监督或离散松弛。 \
**Result**：该方法在解的质量上优于标准非学习近似算法，并展现出极强的规模泛化能力和接近整数线性规划的性能。 \
**Conclusion**：本研究成功将近似算法的理论保证与深度学习的自适应能力相结合，为解决离散优化问题开辟了新方向。
