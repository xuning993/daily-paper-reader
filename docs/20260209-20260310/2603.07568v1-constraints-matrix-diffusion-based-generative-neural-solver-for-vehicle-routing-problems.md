...
tags: ["query:ml-co"]
score: 10.0
evidence: 用于车辆路径问题的生成式神经求解器
tldr: 针对车辆路径问题（VRP），现有自回归神经求解器在处理异构分布、相似节点表示及长决策链时鲁棒性不足。本文提出一种新型融合神经网络框架，利用离散噪声图扩散模型学习底层约束并生成约束分配矩阵。该矩阵作为图结构掩码集成到自回归求解器的特征学习与决策过程中，实现了全局视野与局部特征的有效结合。在CVRPlib数据集上的广泛实验证明，该模型在多种基准测试中达到了当前最优性能，显著提升了神经求解器的泛化能力。
selection_source: fresh_fetch
motivation: 旨在解决现有神经求解器在面对异构分布数据和长决策序列时鲁棒性差、性能退化严重的问题。
method: 提出一种融合框架，利用离散图扩散模型生成约束分配矩阵，并将其作为掩码自适应地集成到自回归求解器的特征表示中。
result: 在CVRPlib公用数据集的378个组合空间维度上进行了全面测试，实验结果表明该模型在多个基准任务中均达到了SOTA水平。
conclusion: 通过将约束感知的扩散模型与自回归架构相结合，可以有效捕捉并利用问题约束，显著增强神经求解器解决复杂VRP问题的效能。
---
