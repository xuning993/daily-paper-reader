...
tags: ["query:ml-co"]
score: 8.0
evidence: 强化学习用于物流和调度中的大规模离散动作空间
...
## 摘要
强化学习正越来越多地应用于物流、调度和推荐系统，但标准算法在处理此类大规模离散动作空间时面临维度灾难的挑战。现有算法通常依赖于受限的网格结构或计算成本高昂的最近邻搜索，这限制了它们在高维或不规则结构领域中的有效性。我们提出了距离引导强化学习（DGRL），结合了采样动态邻域（SDN）和基于距离的更新（DBU），从而在动作数量高达 10^20 的空间中实现高效的强化学习。与以往的方法不同，SDN 利用语义嵌入空间进行随机体积探索，并可证明在局部置信区域内提供全支撑。作为补充，DBU 将策略优化转化为稳定的回归任务，使梯度方差与动作空间基数解耦，并保证策略的单调提升。DGRL 可以自然地推广到混合连续-离散动作空间，且无需层级依赖。我们在规则和不规则结构的场景中证明，与最先进的基准方法相比，该方法的性能提升高达 66%，同时提高了收敛速度并降低了计算复杂度。
