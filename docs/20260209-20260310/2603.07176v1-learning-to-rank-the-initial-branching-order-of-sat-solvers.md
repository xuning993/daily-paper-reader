...
## 摘要
寻找良好的分支顺序是高效解决 SAT 问题的关键，但寻找此类分支顺序本身就是一个难题。因此，在求解之前使用基于学习的方法来预测良好的分支顺序具有潜力。在本文中，我们研究了使用图神经网络（GNN）作为冲突驱动子句学习（CDCL）SAT 求解器的预处理步骤来预测分支顺序。我们表明，通过提供良好的初始分支，现有的 CDCL SAT 求解器可以获得显著的性能提升。此外，我们提供了三种标注方法，以一种易于处理的方式寻找此类初始分支顺序。最后，我们训练了一个图神经网络来预测这些分支顺序，并通过评估表明，GNN 初始化的排序在随机 3-CNF 和伪工业基准测试中产生了显著的加速，并且具有泛化到比训练集大得多的实例的能力。然而，我们也发现预测在加速更困难和工业实例方面效果不佳。我们将此归因于求解器的动态启发式算法，这些算法会迅速覆盖提供的初始化，以及这些实例的复杂性使得 GNN 预测变得困难。
