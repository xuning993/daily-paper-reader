...
## 摘要
多选择背包问题（MCKP）是一类具有广泛实际应用的经典组合优化问题。本文研究了 MCKP 的一个重要但尚未得到充分探索的扩展：隐式概率分布下的多目标机会约束 MCKP（MO-CCMCKP）。该问题的目标是同时最小化总成本并最大化满足容量约束的置信水平，从而捕捉 5G 网络配置等领域中的关键权衡。为了应对在隐式分布下评估机会约束的计算挑战，我们首先提出了一种保序高效资源分配蒙特卡洛（OPERA-MC）方法。该方法自适应地分配采样资源以保持支配关系，同时显著减少评估时间。此外，我们开发了 NHILS，这是一种混合演化算法，它将专门的初始化和局部搜索集成到 NSGA-II 中，以在稀疏可行区域中进行搜索。在合成基准测试和真实 5G 网络配置基准测试上的实验表明，NHILS 在收敛性、多样性和可行性方面始终优于几种最先进的多目标优化器。基准实例和源代码将公开，以促进该领域的研究。
