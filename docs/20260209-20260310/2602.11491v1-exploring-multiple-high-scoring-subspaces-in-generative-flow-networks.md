...
## 速览
**TLDR**：生成流网络（GFlowNets）在处理复杂组合对象时常因搜索空间过大而陷入低奖励区域。本文提出CMAB-GFN框架，将组合多臂老虎机（CMAB）与GFlowNet结合。通过CMAB剪枝低质量动作，构建紧凑的高分搜索子空间，从而在保证样本多样性的同时，显著加速高奖励候选解的发现。实验证明该方法在多个任务上优于现有技术。 \
**Motivation**：现有的GFlowNets在庞大的状态空间中过度探索低奖励区域，导致收敛到次优分布且效率低下。 \
**Method**：提出CMAB-GFN框架，利用组合多臂老虎机机制剪枝低质量动作，将探索限制在多个高分候选子空间内。 \
**Result**：在多项任务上的实验结果表明，CMAB-GFN能够比现有方法生成奖励更高且更具多样性的候选解。 \
**Conclusion**：通过将CMAB的子空间剪枝能力与GFlowNet的采样能力结合，可以有效提升生成模型在复杂组合空间中的搜索效率。
