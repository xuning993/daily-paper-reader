...
## 摘要
我们从神经符号推理的角度研究数学发现。在这一过程中，由大语言模型 (LLM) 驱动的 AI 代理，结合符号计算工具和人类的战略指导，共同在组合设计理论中产生了一项新成果。这一人机协作的主要成果是针对众所周知的难题 $n \equiv 1 \pmod{3}$ 情况，给出了拉丁方不平衡性 (imbalance of Latin squares) 的紧致下界。我们通过跨越数天、多个会话的详细交互日志重构了发现过程，并识别了各组成部分的独特认知贡献。AI 代理在揭示隐藏结构和生成假设方面被证明是有效的。符号组件由计算机代数、约束求解器和模拟退火组成，提供了严格的验证和穷举枚举。人类的引导提供了关键的研究转折点，将死胡同转化为富有成效的探究。我们的分析表明，前沿 LLM 之间的多模型研讨在批评和错误检测方面是可靠的，但在建设性主张方面并不可靠。最终的人机数学贡献——紧致下界 $4n(n{-}1)/9$——是通过一类新型的近乎完美置换 (near-perfect permutations) 实现的。该下界已在 Lean 4 中得到形式化验证。我们的实验表明，神经符号系统确实可以在纯数学领域产生真正的发现。
