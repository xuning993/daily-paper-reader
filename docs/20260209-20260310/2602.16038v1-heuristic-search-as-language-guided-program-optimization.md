---
title: Heuristic Search as Language-Guided Program Optimization
title_zh: 启发式搜索作为语言引导的程序优化
authors: "Mingxin Yu, Ruixiao Yang, Chuchu Fan"
date: 2026-02-17
pdf: "https://arxiv.org/pdf/2602.16038v1"
tags: ["query:ml-co"]
score: 9.0
evidence: 大语言模型驱动的组合优化自动启发式设计
tldr: 针对自动启发式设计中依赖人工干预的问题，本文提出一种将启发式搜索视为语言引导程序优化的结构化框架。该框架将过程分解为评估、反馈和优化三个模块化阶段，实现了系统性的迭代改进。在四个组合优化领域的实验证明，该框架显著优于现有基准，并能通过模块化升级提升多种流行方法的性能。
selection_source: fresh_fetch
motivation: 现有自动启发式设计方法缺乏系统性改进机制，且高度依赖人工调试和领域专家知识。
method: 提出一种模块化框架，将启发式发现过程解耦为前向评估、后向分析反馈和程序优化更新三个阶段。
result: 在四个真实的组合优化领域中，该框架一致优于基准方法，在未见测试集上实现了显著的性能提升。
conclusion: 该框架为 LLM 驱动的启发式设计提供了统一的抽象，通过模块化组件升级可显著增强现有 AHD 方法。
---

## 摘要
近年来，大语言模型（LLMs）推动了组合优化（CO）中自动启发式设计（AHD）的发展。然而，现有的发现流程通常需要大量的手动试错或依赖领域专业知识，以适应新问题或复杂问题。这源于紧密耦合的内部机制，限制了对 LLM 驱动的设计过程进行系统性改进。为了应对这一挑战，我们提出了一个用于 LLM 驱动 AHD 的结构化框架，该框架明确地将启发式发现过程分解为模块化阶段：用于评估的前向传递、用于分析反馈的反向传递以及用于程序细化的更新步骤。这种分离为迭代细化提供了清晰的抽象，并能够对单个组件进行有原则的改进。我们在四个不同的真实世界组合优化领域验证了我们的框架，结果显示它始终优于基准方法，在未见过的测试集上 QYI 提升高达 0.17。最后，我们展示了几种流行的 AHD 方法是我们框架的受限实例化。通过将它们集成到我们的结构化流水线中，我们可以模块化地升级组件并显著提高其性能。

## 速览
**TLDR**：本研究针对大语言模型在自动启发式设计（AHD）中存在的模块耦合及依赖人工干预等问题，提出了一种将启发式搜索视为语言引导程序优化的结构化框架。该框架将设计过程解耦为评估、反馈和更新三个模块化阶段，实现了迭代优化的系统性改进。实验表明，该方法在四个组合优化领域均优于现有基准，不仅提升了性能，还为统一和升级现有 AHD 方法提供了通用范式。 \
**Motivation**：现有的自动启发式设计方法因内部机制耦合严重，往往需要大量人工试错或领域专家知识，难以进行系统性改进。 \
**Method**：提出一种模块化框架，将启发式发现过程显式分解为评估（前向）、分析反馈（后向）和程序精炼（更新）三个阶段。 \
**Result**：在四个真实的组合优化领域中，该框架一致优于基准方法，在未见测试集上的 QYI 指标最高提升了 0.17。 \
**Conclusion**：该框架通过提供清晰的抽象，不仅实现了启发式设计的迭代优化，还能作为通用管道整合并显著提升现有 AHD 方法的性能。

---

## Abstract
Large Language Models (LLMs) have advanced Automated Heuristic Design (AHD) in combinatorial optimization (CO) in the past few years. However, existing discovery pipelines often require extensive manual trial-and-error or reliance on domain expertise to adapt to new or complex problems. This stems from tightly coupled internal mechanisms that limit systematic improvement of the LLM-driven design process. To address this challenge, we propose a structured framework for LLM-driven AHD that explicitly decomposes the heuristic discovery process into modular stages: a forward pass for evaluation, a backward pass for analytical feedback, and an update step for program refinement. This separation provides a clear abstraction for iterative refinement and enables principled improvements of individual components. We validate our framework across four diverse real-world CO domains, where it consistently outperforms baselines, achieving up to $0.17$ improvement in QYI on unseen test sets. Finally, we show that several popular AHD methods are restricted instantiations of our framework. By integrating them in our structured pipeline, we can upgrade the components modularly and significantly improve their performance.