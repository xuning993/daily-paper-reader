...
## 摘要
现代机器学习 (ML) 工作负载日益依赖 GPU，但由于受 GPU 核函数 (kernel) 效率和主机端 (host-side) 设置的双重影响，实现高性能的端到端表现仍具挑战。尽管基于大语言模型 (LLM) 的方法在自动化 GPU 核函数生成方面展现出潜力，但先前的工作主要集中在单核优化，未能扩展到端到端程序，阻碍了实际部署。为了应对这一挑战，本文提出了 StitchCUDA，这是一个用于端到端 GPU 程序生成的多智能体框架，包含三个专门的智能体：负责编排整个系统设计的 Planner（规划者）、致力于逐步实现的 Coder（代码编写者），以及使用 Nsys/NCU 进行正确性检查和性能分析的 Verifier（验证者）。为了从根本上提高 Coder 在端到端 GPU 编程中的能力，StitchCUDA 针对“任务到代码生成”和“反馈驱动的代码优化”这两项原子技能，集成了基于评分量表 (rubric-based) 的智能体强化学习，并将评分量表奖励与来自实际执行的基于规则的奖励相结合。因此，Coder 学会了如何实现高级 CUDA 编程技术（例如自定义核函数融合、cublas epilogue），同时我们也有效防止了 Coder 在基准测试过程中的奖励作弊 (reward hacking) 行为（例如直接复制 PyTorch 代码或硬编码输出）。在 KernelBench 上的实验表明，StitchCUDA 在端到端 GPU 编程任务上实现了近 100% 的成功率，其加速比比多智能体基线高出 1.72 倍，比强化学习模型基线高出 2.73 倍。
