...
tldr: 本研究针对大语言模型在推理阶段适应多样化任务的需求，提出了MASS元学习框架。该框架允许模型通过生成特定问题的合成训练数据并进行针对性自我更新来实现测试时自适应。通过双层优化机制，内循环在合成数据上进行微调，外循环利用元梯度优化数据生成质量，从而在数学推理等任务中实现了高效且具备实例针对性的课程学习与性能提升。
selection_source: fresh_fetch
motivation: 大语言模型在面对多样化领域和任务时，需要在推理阶段具备自我改进和自适应的能力。
method: 提出MASS框架，利用双层优化和元梯度技术，通过生成合成数据进行内循环更新，并根据下游表现优化数据生成策略。
result: 在数学推理任务上的实验表明，MASS能够为每个实例生成有效的学习课程，实现高效的测试时自适应。
conclusion: MASS证明了通过元学习合成数据，大语言模型可以在推理阶段实现有效的自我进化和性能增强。
---

## 摘要
作为强大的通用推理器，大语言模型 (LLMs) 会面临各种领域和任务，其中在测试时进行自适应和自我提升的能力具有重要价值。我们引入了 MASS，这是一个元学习框架，使 LLMs 能够通过生成针对特定问题的合成训练数据，并在推理时执行针对下游性能优化的定向自我更新来实现自适应。我们通过双层优化端到端地训练这种行为：内循环在自生成的样本上进行自适应，而外循环则元学习数据归因信号并奖励更新后的任务性能。合成数据通过可扩展的元梯度进行优化，通过内循环更新反向传播下游损失，从而奖励有用的生成。数学推理实验表明，MASS 学习合成针对每个实例的课程，从而实现有效且数据高效的测试时自适应。
