...
## 摘要
二次无约束二进制优化 (QUBO) 是一个标准的 NP-难优化问题。近年来，由于 QUBO 可以直接归约为量子退火设备所基于的 Ising 模型，它在量子计算领域重新引起了关注。我们引入了一种利用比较-交换网络（compare-exchange networks）的置换 QUBO 表述，仅需 $O(n \log^2 n)$ 个二进制变量。相比于需要 $n^2$ 个变量且交互图更为密集的标准置换矩阵编码，这是一个实质性的改进。我们方法的一个核心特征是均匀性：每个置换都对应唯一的变量赋值，从而实现了无偏采样。我们的构建还允许添加额外的约束，包括不动点和奇偶性。此外，它提供了一种支持乘法和求逆运算的置换表示，并且可以检查置换的阶。这可用于均匀地生成给定阶的置换，或者例如生成与指定置换对易的置换。据我们所知，这是首个将不经意比较-交换网络（oblivious compare-exchange networks）与 QUBO 编码联系起来的研究结果。虽然使用置换矩阵也可以实现类似的功能，但我们的方法生成的 QUBO 规模更小且更稀疏。我们预期该方法在受限置换的无偏采样至关重要的领域（包括密码学和组合设计）具有实际应用价值。
